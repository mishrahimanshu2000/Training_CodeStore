...
const showBookings = async() => {
    ...
    for (let i = 0; i < res.length; i++) {
        ...
        details += `<br>Date: ${res[i].date} <br>Time: ${res[i].time} Table: ${res[i].table} <br>Number of Person: ${res[i].persons} <br>`;
        // console.log(details);
        // console.log(btn);
        p.innerHTML = details;
        p.appendChild(btn);
...
const deleteBooking = async(delId) => {
    ...
    const url = `http://localhost:3000/deleteBooking/${delId.id.toString()}`
